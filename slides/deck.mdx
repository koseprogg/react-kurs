...
Hvordan kan vi lage en Message kompoent vi kan bruke flere ganger, sånn at vi ikke gjenntar oss så mange ganger?

Hent koden som ligger i master på GitHub TODO
...
```diff 1[14:40] title="En hook er funksjon som opererer på React state og 'lifecycle' metoder. Disse funksjonene kan kun brukes inne i funksjonelle komponenter"

```

```diff 4 subtitle="For å lage en state variabel kaller vi useState funksjonen inne i komponenten vår med en default verdi (her 0)"

```

```diff 4[8:26] subtitle="Her bruker vi 'array destructuring' for å definere to variabler som useState lager. Den første er selve staten, og den andre er en funksjon som setter staten vår"

```

```diff 7 subtitle="Nå kan vi bruke count staten vår som en variabel. Akkurat som med props"

```

```diff 8 subtitle="Men, nå har vi en variabel verdi som vi kan endre på! Vi bruker setCount funksjonen til å inkrementere count"
